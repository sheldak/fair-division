...
# agents, items, restrictions = import_from_file(f"instances/stuck_for_global_tie_breaks.txt")
agents, items, restrictions = import_from_file(f"instances/ordered_picking_05_ef1.txt")
# agents, items, restrictions = import_from_file(f"instances/test.txt")
...
#     if is_efx0(agents, allocation) == True:
#         print_allocation(allocation)
# (allocation, items_left) = round_robin(agents, items)
# allocation = xp_ece(agents, items)
# allocation = xp_ece(agents, items, "results/many_ties/results_other/")
